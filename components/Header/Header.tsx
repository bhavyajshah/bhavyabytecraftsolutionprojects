...
export default function Header() {
  ...
  return (
    <>
                              ...
                              <AnimatePresence>
                                {open && (
                                  <PopoverPanel
                                    ...
                                    exit={{ opacity: 0, y: -10 }}
                                    className="absolute left-0 mt-3 p-3 w-full max-w-screen overflow-hidden z-50 bg-[#030014] dark:bg-white rounded-lg shadow-xl origin-top grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 overflow-y-auto max-h-[80vh]"
                                  >
                                    {services.map((service, index) => (
                                      <div
                                        key={index}
                                        className="flex flex-row gap-4 items-start space-y-2 hover:rounded-lg hover:bg-gradient-to-b from-primary-foreground/10 to-transparent dark:from-primary-dark-foreground/10 p-2"
                                      >
        ...
        >
          ...
          <div className="p-6 text-primary-foreground dark:text-primary-dark-foreground">
            ...
            <div className="mt-4 flex flex-col gap-4">
              <Link
                href="/pricing"
                className="button-border-gradient relative rounded-lg text-primary-foreground dark:text-primary-dark-foreground text-sm flex items-center justify-center gap-1.5 py-2 px-4.5 shadow-button hover:button-gradient-hover hover:shadow-none"
                onClick={() => setNavigationOpen(false)}
              >
                <PiHandWaving className="w-6 h-6" />
                Get in Touch
      ...
      {/* Mobile Bottom Menu - Now fixed at the bottom */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-primary dark:bg-primary-dark shadow-lg px-4 py-2 flex justify-around items-center w-full z-50">
        {navLinks.map((item, index) => (
          <Link
            key={index}
            href={item.href}
            className="text-primary-foreground dark:text-primary-dark-foreground p-2 rounded-full hover:bg-primary-foreground/10 dark:hover:bg-primary-dark-foreground/10 transition-all duration-300 flex flex-col items-center"
          >
            {item.icon}
