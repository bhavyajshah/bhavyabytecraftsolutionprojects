...
export default function MaintenanceSupportService() {
    return (
        <div className="min-h-screen text-white my-12 md:p-8">
            <motion.div
            ...
            >
                <h1 className="text-4xl text-white md:text-5xl font-bold mb-4 text-center">IT Maintenance Support Services</h1>
                <p className="text-white text-xl mb-12 text-center max-w-4xl mx-auto">
                ...
                >
                    {supportCategories.map((category, index) => (
                        <motion.div key={index} variants={itemVariants}>
                            <Card className="backdrop-blur-sm bg-gray-900/30 border-gray-700 overflow-hidden group hover:shadow-2xl transform transition-transform duration-500 hover:-translate-y-2">
                                <CardHeader className="relative p-6">
                                    ...
                                    <CardTitle className="text-2xl text-white font-semibold group-hover:text-white transition-colors duration-300">
                                        {category.title}
                                    </CardTitle>
                                    <CardDescription className="text-white mt-2">{category.description}</CardDescription>
                                </CardHeader>
                                <CardContent className="p-6">
                                    <ul className="space-y-3">
                                        {category.services.map((service, serviceIndex) => (
                                            ...
                                            >
                                                <motion.div
                                                    className="w-2.5 h-2.5 text-white rounded-full mr-2"
                                                    initial={{ scale: 0 }}
                                                    animate={{ scale: 1 }}
