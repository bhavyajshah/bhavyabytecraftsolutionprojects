...
export function CreativePriceRequestForm() {
    ...
    return (
        <div className="min-h-screen w-full text-gray-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
            <div className="relative z-10 max-w-6xl mx-auto">
                <div className="border-[#6430c2] border rounded-2xl shadow-[#6430c2] overflow-hidden">
                    <div className="px-8 py-6 bg-[#6430c2]">
                        <h1 className="text-4xl font-extrabold text-white mb-2">Request Project Pricing</h1>
                        <p className="text-purple-200">Let&apos;s craft your perfect software solution</p>
                    </div>

                    <form onSubmit={formik.handleSubmit} className="space-y-8 p-8">
                        <div>
                            <h2 className="text-xl font-semibold mb-4 flex items-center">
                                <svg className="w-6 h-6 mr-2 text-[#6430c2]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {['Web', 'Mobile', 'Consultancy'].map((option) => (
                                    <div key={option} className="flex items-center space-x-3 bg-gray-700 p-4 rounded-lg">
                                        <Checkbox
                                            id={option}
                                            ...
                                            className="border-purple-400 text-[#6430c2]"
                                        />
                            ...
                            {formik.touched.softwareSolution && formik.errors.softwareSolution && (
                                <div className="text-red-400 text-sm mt-2">{(formik.errors as any).softwareSolution}</div>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            >
                                {['Idea', 'Prototype', 'Designed', 'MVP'].map((option) => (
                                    <div key={option} className="flex items-center space-x-3 bg-gray-700 p-4 rounded-lg">
                                        <RadioGroupItem value={option} id={option} className="border-purple-400 text-purple-600" />
                                        <Label htmlFor={option} className="text-lg">{option}</Label>
                                    </div>
                                ))}
                            </RadioGroup>
                            {formik.touched.currentStage && formik.errors.currentStage && (
                                <div className="text-red-400 text-sm mt-2">{formik.errors.currentStage}</div>
                            )}
                        </div>
                        ...
                        <Button type="submit" className="w-full bg-[#6430c2] text-white font-bold py-3 px-4 rounded-lg text-lg transition duration-300 ease-in-out transform hover:scale-105">
