...
const CostCard: React.FC<{ title: string; value: number; icon: React.ElementType; isAfter?: boolean }> = ({ title, value, icon: Icon, isAfter = false }) => (
  <div className={`flex items-center p-6 rounded-2xl ${isAfter ? 'bg-green-900/20' : 'bg-red-900/20'} backdrop-blur-sm shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl`}>
    <Icon className={`w-10 h-10 mr-4 ${isAfter ? 'text-green-400' : 'text-red-400'}`} />
    <div>
      <h3 className="text-lg font-semibold text-gray-200">{title}</h3>
      <p className={`text-2xl font-bold ${isAfter ? 'text-green-400' : 'text-red-400'}`}>
        {value}%
      </p>
    </div>
  </div>
...
export default function AICostSavings() {
  ...
  return (
    <div className="min-h-screen text-gray-100 p-4 sm:p-6 mb-20 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <div className="relative inline-block">
            <svg className="w-48 h-48" viewBox="0 0 100 100">
              ...
              ></circle>
              <circle
                className="text-[#6430C2] progress-ring__circle stroke-current"
                strokeWidth="10"
                strokeLinecap="round"
                cx="50"
                cy="50"
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-5xl font-bold text-[#6430C2]">{animatedPercentage}%</span>
            </div>
          </div>
          <p className="text-2xl font-semibold text-[#6430C2] mt-4">Average Cost Reduction Across Industries</p>
        </div>

        <div className="mb-12 backdrop-blur-sm bg-gray-900/30 rounded-3xl shadow-2xl p-6">
          <div className="flex flex-col sm:flex-row border-b border-gray-700 mb-6">
            <button
              className={`flex-1 py-3 px-4 focus:outline-none transition-colors duration-300 ${activeTab === 'operational'
                ? 'border-b-2 border-[#6430C2] text-[#6430C2] font-semibold'
                : 'text-gray-400 hover:text-[#6430C2]'
                }`}
        ...
        <div className="mt-16 mb-12 backdrop-blur-sm bg-gray-900/30 rounded-3xl shadow-2xl p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4">
            {technologyFields.map((field, index) => (
              <button
                key={index}
                className={`p-4 rounded-xl transition-all duration-300 flex flex-col items-center justify-center ${selectedField === field.name ? 'bg-[#6430C2] text-white' : 'backdrop-blur-sm bg-gray-900/30 text-gray-300 hover:bg-gray-600'
                  }`}
          ...
          {selectedField && (
            <div className="mt-8 p-6 backdrop-blur-sm bg-gray-900/30 rounded-xl">
        ...
        <div className="mt-16">
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-2">
            {successStories.map((story, index) => (
              <div key={index} className="backdrop-blur-sm bg-gray-900/30 shadow-xl rounded-3xl p-6 transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">

                <div className="flex items-center mb-4">
                  <FaBolt className="w-8 h-8 text-yellow-400 mr-3" />
                  <h3 className="font-semibold text-2xl text-gray-200">{story.company}</h3>
                </div>
                <p className="text-blue-400 font-semibold mb-2">{story.field}</p>
                <div className="space-y-4">
                  <div>
                    <p className="font-semibold text-red-400 mb-2">Before AI:</p>
                    <p className="text-gray-300">{story.before}</p>
                  </div>
                  <div>
                    <p className="font-semibold text-green-400 mb-2">After AI:</p>
                    <p className="text-gray-300">{story.after}</p>
                  </div>
                  <div>
                    <p className="font-semibold text-yellow-400 mb-2">Savings:</p>
                    <p className="text-gray-300">{story.savings}</p>
                  </div>
                </div>
              </div>
            ))}
